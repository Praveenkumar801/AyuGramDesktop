...
, _globalViewport(std::move(viewportValue)) {
...

	{
		const auto close = Ui::CreateChild<Ui::RoundButton>(
			this,
			item->out() || settings.saveDeletedMessagesFor(item->history()->peer->id.value)
				? tr::lng_close()
		...
		auto text = item->out()
			? (isRound
				...
				: tr::lng_ttl_voice_tooltip_out)(
					...
					tr::rich)
			: (isRound
				? settings.saveDeletedMessagesFor(item->history()->peer->id.value) ? tr::ayu_ExpiringVideoMessageNote : tr::lng_ttl_round_tooltip_in
				: settings.saveDeletedMessagesFor(item->history()->peer->id.value) ? tr::ayu_ExpiringVoiceMessageNote : tr::lng_ttl_voice_tooltip_in)(tr::rich);
